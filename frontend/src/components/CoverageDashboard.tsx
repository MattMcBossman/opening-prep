...
import { aggregatePositionCoverage, calculatePositionCoverage, opponentPositions } from '../lib/repertoireCoverage'
...
export function CoverageDashboard({ color, tree, apiToken, signedIn, filters, getContinuations }: Props) {
    ...
    const run = async () => {
      ...
      for (let index = 0; index < positions.length; index += 1) {
        ...
        try {
          ...
          next.push(calculatePositionCoverage(stats.moves, getContinuations(fen), getContinuations))
          setScores([...next])
          setProgress(index + 1)
  ...
  return <section className="coverage-dashboard">
    ...
    {!requested ? <>
      ...
      <p className="panel-status">{summary.coveredPositions}/{summary.totalPositions} scored positions fully covered.</p>
