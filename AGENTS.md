...
- **Audio**: move sounds are synthesized at runtime with the Web Audio API (`src/audio/soundPlayer.ts`), not shipped as audio files — no binary assets to license, bundle, or fetch, and each cue is a handful of tunable numbers. Four distinct cues (regular move, capture, check, checkmate), chosen from the move's SAN (`src/lib/moveSounds.ts`) with precedence checkmate > check > capture > move, so a move that is several at once only plays its most significant cue. A single shared `AudioContext` is created lazily on the first cue, since browsers suspend contexts not created during a user gesture. Sounds fire only for moves that actually land on the board — in drills, a rejected attempt stays silent. Swapping in sampled recordings later should only touch `soundPlayer.ts`.
