...
Pass `--stockfish /path/to/stockfish` to analyze the most popular candidates
and choose the most popular move within `--max-engine-loss-cp` of the best
evaluated candidate. `--engine-depth` controls analysis depth.
